...
def local_setup_logger(OUTPUT_NAME):
    ...
    # ログファイルの記録先
    Log_file_path = os.path.join(BUILD_FONTS_DIR, f"{OUTPUT_NAME}finishserch.log")
...
def custom_stroke_width(glyph, font_weight, base_stroke_width, posiflag):
    ...
    # ストローク幅が10を切るならposiflagをFalseにする
    if stroke_width < 10:
        posiflag = False
    return point_count, stroke_width, posiflag
...
def main():
    ...
    for glyph in font.glyphs():  # 全グリフをループ処理
        ...
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Start processing          \r", end=" ", flush=True)
        ...
        point_count, stroke_width, posiflag = custom_stroke_width(glyph, font_weight, base_stroke_width, posiflag)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Point Count {point_count:<6}        \r", end=" ", flush=True)
        upscale(glyph, wratio)  # 縦に拡大する(強制モノスペース版はこの際に幅も広げる)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Wider stroke              \r", end=" ", flush=True)
        # ストロークによる拡幅処理を実行する。
        if posiflag: ys_widestroke(glyph, stroke_width, STOROKE_HEIGHT, proc_cnt)
        # 幅を縮小＆最初に縦に伸ばした奴を元に戻す
        downscale(glyph)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Anomality Repair          \r", end=" ", flush=True)
     #   if posiflag: anomality_repair(glyph)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Anomality Repair Plus     \r", end=" ", flush=True)
        # 変形、精度劣化を伴う修復試行を行う。
    #    if glyph.validate(1) & 0x20: ys_rescale_chain(glyph)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Overflow treatment        \r", end=" ", flush=True)
        # ストロークで太ったアウトラインのを引き締めてオリジナルの幅に近付ける
     #   if posiflag: shapeup_outline(glyph, em_size, stroke_width)
        vshrink(glyph)  # 指定の縮小率に従って縦横比変更
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Finish optimization       \r", end=" ", flush=True)
    #    finish_optimise(glyph, proc_cnt)
        # Local_validate_notice(glyph, "仕上げ処理後", "warning")  # 仕上げ後の検査(デバッグ用)
        print(f"now:{proc_cnt:<5}:{glyph.glyphname:<15} Process Completed!        \r", end=" ", flush=True)
    ######################################################################
    #                    各グリフのループ処理ここまで                    #
    ######################################################################
    # SFDファイルの保存
