...
def ys_repair_Self_Insec(glyph, angle_threshold=2):
    ...
    for contour in glyph.foreground:
        ...
        while i < num_on_curve:  # 取得したインデックスから繰り返す
            ...
            # ベクトルを計算
            prev_coords = (prev_point.x - current_point.x, prev_point.y - current_point.y)
            next_coords = (next_point.x - current_point.x, next_point.y - current_point.y)

            # 内積と外積を計算
            dot_product = prev_coords[0] * next_coords[0] + prev_coords[1] * next_coords[1]
            cross_product = prev_coords[0] * next_coords[1] - prev_coords[1] * next_coords[0]

            # 角度を計算（符号付き）
            angle = math.atan2(cross_product, dot_product)
            if angle < 0:
                angle += 2 * math.pi  # 角度を0から2πの範囲に調整
            ...
            if not (angle_threshold_rad <= angle <= (two_pi - angle_threshold_rad)):
                # 問題のあるポイントの連続を検出
                ...
                while True:
                    ...
                    # 角度の評価
                    if (angle_threshold_rad <= angle <= (two_pi - angle_threshold_rad)):
                        break  # 問題のない角度が現れたら終了
