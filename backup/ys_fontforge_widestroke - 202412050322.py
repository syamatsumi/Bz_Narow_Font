...
def ys_expand_Xweight(glyph, offset):
    ...
    # コンターを一つずつ書き出して加工、格納(追加)を繰り返す
    wroking_contour = fontforge.contour(True)
    contour_processe = fontforge.contour(True)  # 処置済みコンターの格納場所
    for contour in glyph_backup:
        ...
        # 処理が済んだパスを保存。
        contour_processe += working_contour.dup()

    # フォアグラウンドをクリアして、
    # 処理済みコンターをフォアグラウンドに戻す
    glyph.foreground = fontforge.layer()
    tmp_cont = fontforge.contour(True)
    for contour in contour_processe:
        tmp_cont += contour.dup()
        glyph.foreground += tmp_cont.dup()
