...
def ys_widestroke(stroke_width, storoke_height, glyph):
        ...
        if glyph.validate(1) & 0x20:
            ...
            if glyph.validate(1) & 0x20:
                ...
                if glyph.validate(1) & 0x20:
                    ...
                    if glyph.validate(1) & 0x20:
                        print(f"\r now:{glyph.glyphname:<15} Repairing self-intersections 6 ", end=" ", flush=True)
    ...
    if glyph.validate(1) & 0x01:
        print(f"\r now:{glyph.glyphname:<15} Repairing an open path 2       ", end=" ", flush=True)
        ys_closepath(glyph)
    if glyph.validate(1) & 0x20:
        print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 1 ", end=" ", flush=True)
        ys_repair_Self_Insec(glyph, 1) 
        glyph_backup = [contour for contour in glyph.foreground]

        if glyph.validate(1) & 0x20:
            print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 2 ", end=" ", flush=True)
            ys_repair_Self_Insec(glyph, 2)
            if glyph.validate(1) & 0x20:
                print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 3 ", end=" ", flush=True)
                ys_repair_Self_Insec(glyph, 3)
                if glyph.validate(1) & 0x20:
                    print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 4 ", end=" ", flush=True)
                    ys_repair_Self_Insec(glyph, 4)
                    if glyph.validate(1) & 0x20:
                        print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 5 ", end=" ", flush=True)
                        ys_repair_Self_Insec(glyph, 5)
                        if glyph.validate(1) & 0x20:
                            print(f"\r now:{glyph.glyphname:<15} Repairing self-intersection2 6 ", end=" ", flush=True)
                            ys_repair_Self_Insec(glyph, 6)
                            if glyph.validate(1) & 0x20:
                                print(f"\r now:{glyph.glyphname:<15} Repair2 failure, rollback...   ", end=" ", flush=True)
                                glyph.foreground = fontforge.layer() # フォアグラウンドをクリア
                                for contour in glyph_backup:  # 保存していたパスの書き戻し
                                    glyph.foreground += contour  # 修復試行前に戻す
        glyph.removeOverlap()  # 結合
