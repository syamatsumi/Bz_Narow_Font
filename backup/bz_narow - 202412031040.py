...
# 読み込むフォントのディレクトリまで確定させる
    ...
    print (f"\r input_fontfile", end=" ", flush=True)
...
# SFDに変換して一旦TTFを閉じる
    ...
    print(f"\r SFDに変更して再開する。file: {temp_file_path}", end=" ", flush=True)
    font.close() # TTFフォントを閉じる

# SFDで保存した一時ファイルを開き直す
    font = fontforge.open(temp_file_path)
    print(f"\r SFDを開きなおした。 file: {temp_file_path}", end=" ", flush=True)
...
######################################################################
    ...
    # 縦に拡大する
        print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Expand in height dir     ", end=" ", flush=True)
        glyph.transform((1, 0, 0, 8, 0, 0))  # 縦に伸ばす
        glyph.addExtrema("all") # 極点を追加

    # 拡幅処理
        print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Wider stroke             ", end=" ", flush=True)
    ...
    # 修復を試みる
        print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Anomality Repair         ", end=" ", flush=True)
        glyph.round()  # 整数化
        glyph.removeOverlap()
    ...
    # 変形、精度劣化を伴う修復試行を行う。
        if glyph.validate(1) != 0:  # どれか一つでも引っかかった場合
            print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Anomality Repair Plus    ", end=" ", flush=True)
            ys_rescale_chain(glyph)

    # 幅のストロークで太ったアウトラインを引き締める。
    # サイドベアリングを弄りたくないので拡大縮小の原点はグリフの中心にする。
    # グリフの幅まで変わると困るのでレイヤー単位で操作
        print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Overflow treatment       ", end=" ", flush=True)
    ...
    # スペースみたいなコンターの無いグリフは元に戻す。
        if style_is_prop:
            print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} proportional processing  ", end=" ", flush=True)
            glyph.round()
    ...
    # 最後にTTFの仕様に合わせた最適化を実施
        print(f"\r now:{proc_cnt:<5}:{glyph.glyphname:<15} Finish optimization      ", end=" ", flush=True)
        if glyph.validate(1) & 0x01:  # 開いたパスがある場合
...
######################################################################
    ...
    try:
        ...
        if del_file != f"temp_0_{OUTPUT_NAME}.sfd":
            ...
            print(f"\r 前の一時ファイルを削除： {del_file_path}", end=" ", flush=True)
...
def Local_snapshot_sfd (savefreq, proc_cnt, del_file, OUTPUT_NAME, font, glyph):
    ...
    # savefreq個処理してたら一旦保存
        if proc_cnt % savefreq == 1:
            print(f"\r 作業前保存中のグリフ： {glyph.glyphname}", end=" ", flush=True)
            temp_file = f"{OUTPUT_NAME}_temp_{proc_cnt}.sfd"
            temp_file_path = os.path.join(BUILD_FONTS_DIR, temp_file)
            font.save(temp_file_path)
        # 前回の仮保存ファイルを削除
            print(f"\r 前の一時ファイルを削除： {del_file}", end=" ", flush=True)
            del_file_path = os.path.join(BUILD_FONTS_DIR, del_file)
            os.remove(del_file_path)
        # 次の削除対象ファイル名を保存
            del_file = temp_file
        ...
        print(f"保存か削除に失敗したかも？　多分削除に……： {del_file}", flush=True)
