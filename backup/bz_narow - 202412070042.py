...
def main():
    ...
    for glyph in font.glyphs():  # 全グリフをループ処理
        ...
        posiflag = True  # 白抜き文字系のグリフで除外したい処理のためのフラグを建てる
        posiflag = ys_list_invglyph(glyph.glyphname, posiflag)  # フラグの判定
        # glyph.background = glyph.foreground  # バックグラウンドにグリフをコピー
        # 強制的に全グリフの幅を揃えるパターン専用の処理
        if INPUT_FONTSTYLES.startswith("M"):
            # グリフ幅が極端に狭いフォントはなにもしない
            if glyph.width < em_size / 5:
                 posiflag = False
        ...
        upscale(glyph, wratio)  # 縦に拡大する(強制モノスペース版は幅も広げる)
