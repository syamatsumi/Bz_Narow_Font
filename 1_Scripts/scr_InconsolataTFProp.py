...
# アウトラインの幅がグリフ幅-最小マージンx2より小さい時
    if content_width < glyph.width - min_margin * 2:
    # かつ左ベッタリだったときは右サイドベアリングを足した幅をグリフ幅にする
        if xmin < min_margin:
            glyph.width = round(xmin + xmax + min_margin)
    # 右が飛び出して居る時は左マージンまで寄せて幅寄せ
        elif rsbearing < 0:
            glyph.transform(psMat.translate(min_margin - xmin, 0))
            glyph.width = round(xmax + min_margin + rsbearing)
    # 飛び出し無しで右ベッタリだった時は上記の補償抜き
        elif rsbearing < min_margin:
            glyph.transform(psMat.translate(min_margin - xmin , 0))
            glyph.width = round(xmax + min_margin)
    # どっちつかずに狭いだけのグリフは両側にマージンを足す
        else:
            glyph.transform(psMat.translate(min_margin - xmin , 0))
            glyph.width = round(content_width + min_margin * 2)
